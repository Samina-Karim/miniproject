...
currenciesRouter.post('/', (request, response) => {
  ...
  if ((currencyCode ==undefined) || (country==undefined) || (conversionRate == undefined)){
...
/** Checks if currencyCode already present then it will not post add and will return with an error message */
  const isCurrencyPresent= currencies.find(currency => currency.currencyCode == currencyCode);
